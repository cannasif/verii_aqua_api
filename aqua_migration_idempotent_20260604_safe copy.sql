...
BEGIN
    IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_RII_GoodsReceipt_WarehouseCode_BranchCode' AND object_id = OBJECT_ID(N'[dbo].[RII_GoodsReceipt]'))
    BEGIN
...
BEGIN
    IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'UX_RII_WeatherSeverity_WeatherType_Code_Active' AND object_id = OBJECT_ID(N'[dbo].[RII_WeatherSeverity]'))
    BEGIN
        DROP INDEX [UX_RII_WeatherSeverity_WeatherType_Code_Active] ON [dbo].[RII_WeatherSeverity];
    END;
END;
GO
...
BEGIN
    IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_RII_WeatherSeverity_WeatherTypeId' AND object_id = OBJECT_ID(N'[dbo].[RII_WeatherSeverity]'))
    BEGIN
        DROP INDEX [IX_RII_WeatherSeverity_WeatherTypeId] ON [dbo].[RII_WeatherSeverity];
    END;
END;
GO
...
BEGIN
    ...
    IF COL_LENGTH('dbo.RII_WeatherSeverity', 'WeatherTypeId') IS NOT NULL
    BEGIN
        ALTER TABLE [dbo].[RII_WeatherSeverity] DROP COLUMN [WeatherTypeId];
    END;
END;
GO
