...
BEGIN
    IF OBJECT_ID(N'[RII_WeatherSeverity]', N'U') IS NOT NULL
    BEGIN
        IF EXISTS (SELECT 1 FROM sys.indexes WHERE [name] = N'UX_RII_WeatherSeverity_Code_Active' AND [object_id] = OBJECT_ID(N'[RII_WeatherSeverity]'))
            DROP INDEX [UX_RII_WeatherSeverity_Code_Active] ON [RII_WeatherSeverity];

        IF COL_LENGTH(N'[RII_WeatherSeverity]', N'WeatherTypeId') IS NULL
            ALTER TABLE [RII_WeatherSeverity] ADD [WeatherTypeId] bigint NULL;

        IF COL_LENGTH(N'[RII_WeatherSeverity]', N'WeatherTypeId') IS NOT NULL
           AND OBJECT_ID(N'[RII_WeatherType]', N'U') IS NOT NULL
           AND ((SELECT COUNT(1) FROM [RII_WeatherType] WHERE [IsDeleted] = 0) = 1)
        BEGIN
            EXEC(N'
                UPDATE ws
                SET [WeatherTypeId] = (
        ...
        IF COL_LENGTH(N'[RII_WeatherSeverity]', N'WeatherTypeId') IS NOT NULL
           AND NOT EXISTS (SELECT 1 FROM sys.indexes WHERE [name] = N'IX_RII_WeatherSeverity_WeatherTypeId' AND [object_id] = OBJECT_ID(N'[RII_WeatherSeverity]'))
            EXEC(N'CREATE INDEX [IX_RII_WeatherSeverity_WeatherTypeId] ON [RII_WeatherSeverity] ([WeatherTypeId]);');

        IF COL_LENGTH(N'[RII_WeatherSeverity]', N'WeatherTypeId') IS NOT NULL
           AND NOT EXISTS (SELECT 1 FROM sys.indexes WHERE [name] = N'UX_RII_WeatherSeverity_WeatherType_Code_Active' AND [object_id] = OBJECT_ID(N'[RII_WeatherSeverity]'))
        ...
        IF COL_LENGTH(N'[RII_WeatherSeverity]', N'WeatherTypeId') IS NOT NULL
           AND OBJECT_ID(N'[RII_WeatherType]', N'U') IS NOT NULL
           AND NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE [name] = N'FK_RII_WeatherSeverity_RII_WeatherType_WeatherTypeId')
            EXEC(N'ALTER TABLE [RII_WeatherSeverity] ADD CONSTRAINT [FK_RII_WeatherSeverity_RII_WeatherType_WeatherTypeId] FOREIGN KEY ([WeatherTypeId]) REFERENCES [RII_WeatherType] ([Id]) ON DELETE NO ACTION;');
    END;
...
BEGIN
    IF OBJECT_ID(N'[RII_WeatherSeverity]', N'U') IS NOT NULL
    BEGIN
        IF EXISTS (SELECT 1 FROM sys.foreign_keys WHERE [name] = N'FK_RII_WeatherSeverity_RII_WeatherType_WeatherTypeId')
            ALTER TABLE [RII_WeatherSeverity] DROP CONSTRAINT [FK_RII_WeatherSeverity_RII_WeatherType_WeatherTypeId];

        IF EXISTS (SELECT 1 FROM sys.indexes WHERE [name] = N'UX_RII_WeatherSeverity_WeatherType_Code_Active' AND [object_id] = OBJECT_ID(N'[RII_WeatherSeverity]'))
            DROP INDEX [UX_RII_WeatherSeverity_WeatherType_Code_Active] ON [RII_WeatherSeverity];

        IF EXISTS (SELECT 1 FROM sys.indexes WHERE [name] = N'IX_RII_WeatherSeverity_WeatherTypeId' AND [object_id] = OBJECT_ID(N'[RII_WeatherSeverity]'))
            DROP INDEX [IX_RII_WeatherSeverity_WeatherTypeId] ON [RII_WeatherSeverity];

        IF COL_LENGTH(N'[RII_WeatherSeverity]', N'WeatherTypeId') IS NOT NULL
        BEGIN
            DECLARE @weatherTypeDefaultConstraint sysname;
            SELECT @weatherTypeDefaultConstraint = [d].[name]
            FROM [sys].[default_constraints] [d]
            INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
            WHERE ([d].[parent_object_id] = OBJECT_ID(N'[RII_WeatherSeverity]') AND [c].[name] = N'WeatherTypeId');
            IF @weatherTypeDefaultConstraint IS NOT NULL EXEC(N'ALTER TABLE [RII_WeatherSeverity] DROP CONSTRAINT [' + @weatherTypeDefaultConstraint + '];');
            EXEC(N'ALTER TABLE [RII_WeatherSeverity] DROP COLUMN [WeatherTypeId];');
        END;

        IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE [name] = N'IX_RII_WeatherSeverity_Code_Active' AND [object_id] = OBJECT_ID(N'[RII_WeatherSeverity]'))
